...
class BaseModel(object):
  ...
  def __init__(self,
               hparams,                             # 超参数
               mode,                                # 模式，可选train、eval、infer
               iterator,                            # 数据迭代器
               source_vocab_table,                  # 源数据的单词到ID的映射表
               target_vocab_table,                  # 目标数据的单词到ID的映射表
               reverse_target_vocab_table=None,     # 是否构造反向映射表，ID到单词的映射表
               scope=None,                          # 变量的域名
               extra_args=None):                    # 额外的参数，如控制分布式训练的参数
    """Create the model.
...
# 1. 获取超参数
  def _set_params_initializer(self,
    ...
    # 数据迭代器，不管是train, eval还是infer，都有一个数据迭代器, 不断地喂数据
    ...
    # 设置编码器和解码器的残差网络层数
    # 残差网络(Residual Network)的作用：解决了深网络的梯度消失和梯度爆炸的问题，同时改善了网络的性能
...
# 解码器
    ...
    with tf.variable_scope("decoder") as decoder_scope:
      ...
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        ...
        my_decoder = tf.contrib.seq2seq.BasicDecoder(
            cell,
            helper,
            decoder_initial_state,)             # 将源词中的隐藏态传递到解码器，作为输入
      ...
      ## Inference
        ...
        outputs, final_context_state, _ = tf.contrib.seq2seq.dynamic_decode(
            ...
            output_time_major=self.time_major,              # 限制翻译的长度，由于无法提前知道目标语句的长度
            swap_memory=True,
            scope=decoder_scope)
  ...
  def _compute_loss(self, logits, decoder_cell_outputs):
    ...
    # target_weights是一个与decoder_outputs大小一样的0 - 1矩阵
    # 该矩阵将目标序列长度以外的其他位置填充为标量值0
    # 训练损失可以由 batch_size 分割
    ...
    with tf.variable_scope("encoder") as scope:
      ...
      if hparams.encoder_type == "uni":
        ...
        cell = self._build_encoder_cell(hparams, num_layers,
                                        num_residual_layers)
# 编码器
        # 词嵌入之后得到新的输入encoder_emb_inp，它将被传入 Encoder中
        # encoder_outpus: [max_time, batch_size, num_units]
        # encoder_state: [batch size, num units] 相当于上下文表示向量context
        # 将新建的RNN展开时间维度
  ...
  def _build_bidirectional_rnn(self, inputs, sequence_length,
    ...
    bi_outputs, bi_state = tf.nn.bidirectional_dynamic_rnn(
        ...
        inputs,
        dtype=dtype,
        sequence_length=sequence_length,                # dynamic_rnn 精确的句子长度
        time_major=self.time_major,                     # 输入是以时间为主
        swap_memory=True)

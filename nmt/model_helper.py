...
def _create_or_load_embed(embed_name, vocab_file, embed_file,
  ...
  if vocab_file and embed_file:
    ...
    # 重新创建词嵌入表示
    # vocab_size：词典的大小，也就是二维矩阵的行数
    # embed_size：词嵌入的维度，每个词用多少个数字来表示，也就是二维矩阵的列数
    # 默认参数trainable=True：训练过程中，变量是可变的
...
def _cell_list(unit_type, num_units, num_layers, num_residual_layers,
  ...
  # Multi-GPU
  cell_list = []
  # 根据num_layers，调用最终的single_cell_fn生成单个cell
...
def gradient_clip(gradients, max_gradient_norm):
  ...
  # 使用全局范数进行截断操作
  # 最大值 max_gradient_norm 通常设置为 5 或 1
